...
class CoverSection(BaseSection):
    ...
    def render(self) -> list[Flowable]:
        ...
        # Title and subtitle
        elements.append(Paragraph(self._s("cover_title"), st["CoverTitle"]))
        elements.append(Spacer(1, 8))
        elements.append(
            Paragraph(self._s("cover_subtitle"), st["CoverSubtitle"])
        )
        elements.append(Spacer(1, 24))

        # Metadata table
        meta_data = [
            [self._s("cover_company"), company_name],
            [self._s("cover_version"), f"v{version}"],
            [self._s("cover_date"), date_str],
            [self._s("cover_classification"), confidentiality],
        ]
